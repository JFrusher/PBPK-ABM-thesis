...
%┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
    ...
    % Get simulation time range (in minutes)
    timeStep = 0.05; 
    if (max(dosingRegimen.end_time_min) <= 1440)
        maxTime = max(dosingRegimen.end_time_min) + 180;
    else
        maxTime = max(dosingRegimen.end_time_min);
    end
    ...
    logger.info('simulation_started', struct('timeStep', timeStep, 'maxTime', maxTime, 'nTimePoints', nTimePoints));
    
    try
        for t = 2:nTimePoints
            currentTime = time_min(t-1); % Use start of step for rate calculation
            dt_actual = timeStep;
        
        % Calculate circadian-modulated DPD activity
        hourOfDay = mod(currentTime, 1440) / 60; % Extract hour from total minutes
        DPD_circadian_factor = calculateCircadianDPD(hourOfDay, params);
        
        % Get current dosing rate (if any active dosing at this time point)
        dosingRate = calculateDosingRate(currentTime, dosingRegimen);

        % ═══════════════════════════════════════════════════════════════════════
        % SUPER-CRITICAL: Log bolus at t=0-1 minute
        % ═══════════════════════════════════════════════════════════════════════
        if t == 2 % First iteration (initial step)
            % Compact structured diagnostic only (avoid verbose console dumps in prod)
        ...
        dCdt = calculate5FUSystemODEs(concentrations, t-1, dosingRate, ...
            DPD_circadian_factor, params, currentTime, logger);
        
        % Update concentrations using Euler method with CORRECT dt
        % C(t) = C(t-1) + dC/dt * dt
    ...
    validation_results = validateAgainstLiterature(results.metrics);
    results.validation = validation_results;
        
    % Save detailed CSV outputs
    saveDetailedCSVOutputs(results, outputPrefix);
    
    % Generate comprehensive plots XXX TURN THIS BACK ON
    %generateComprehensivePlots(results, outputPrefix);
    
    % Print summary statistics
    printSummaryStatistics(results);
    logger.info('simulation_output_packaged', struct('outputPrefix', outputPrefix));
catch ME
    logger.fatal('simulation_failed', struct('message', ME.message, 'identifier', ME.identifier, 'time', now));
    rethrow(ME);
end
...
% ================================================================================
    ...
    params.V_liver = 0.026;      % Liver volume (1.8 L)
    params.V_kidney = 0.004;     % Kidney volume (0.31 L)
    params.V_heart = 0.005;      % Heart volume (0.33 L)
    params.V_brain = 0.02;       % Brain volume (1.4 L)
    
    % Peripheral compartment (poorly perfused)
    params.V_muscle = 0.4;       % Muscle volume (28 L)
    params.V_fat = 0.2;          % Fat volume (14 L)
    params.V_skin = 0.05;        % Skin volume (3.5 L)
    
    % Tumor compartment
    params.V_tumor = 0.0005;     % Tumor volume (35 mL, ~3 cm diameter solid tumor)
    
    %% BLOOD FLOW DISTRIBUTION (fraction of cardiac output)
    
    % Well-perfused organs
    params.Q_liver = 0.25;       % Hepatic blood flow (25% of CO = 1.625 L/min)
    params.Q_kidney = 0.19;      % Renal blood flow (19% of CO = 1.235 L/min)
    params.Q_heart = 0.04;       % Cardiac blood flow (4% of CO)
    params.Q_brain = 0.12;       % Cerebral blood flow (12% of CO)
    ...
    % Tumor - well perfused assumption (similar to kidney/liver)
    % Konings et al. (2010): Tumor concentrations reach 61% of plasma
    params.Q_tumor = 0.02;       % Tumor blood flow (2% of CO, well-vascularized)
    ...
    params.V_peripheral = params.BW * 0.3;  % Peripheral volume ~21 L
    
    % Partition coefficients (tissue:plasma ratio)
    ...
    % - Vmax: 1220 mg/h whole-liver value (not naive scaling)
    % - Km: 5.0 mg/L in vivo value (not 25, not 0.5 cytosolic)
    % - This combination produces AUC = 20-30 mg·h/L (correct)
    % ═══════════════════════════════════════════════════════════════════════════════
...
% ================================================================================
    fprintf('\n────────────────────────────────────────────────────────────\n');
    fprintf('VALIDATING CLEARANCE PARAMETERS (Well-Stirred Model)\n');
    fprintf('────────────────────────────────────────────────────────────\n\n');

    % 1. Calculate Intrinsic Clearance (Theoretical max speed)
    % CL_int = Vmax / Km
    CL_int_L_min = params.Vmax_DPD / params.Km_DPD; 
    
    fprintf('Intrinsic Clearance (Enzyme capacity): %.2f L/min\n', CL_int_L_min);

    % 2. Calculate Hepatic Clearance (Limited by blood flow)
    % Formula: CL_hep = (Q * CL_int) / (Q + CL_int)
    Q_liver_L_min = params.Q_liver * params.CO; % ~1.6 L/min
    CL_hepatic_L_min = (Q_liver_L_min * CL_int_L_min) / (Q_liver_L_min + CL_int_L_min);
    
    fprintf('Hepatic Blood Flow (Physiological limit): %.2f L/min\n', Q_liver_L_min);
    fprintf('Actual Hepatic Clearance: %.2f L/min\n', CL_hepatic_L_min);

    % 3. Calculate Total Systemic Clearance
    % Add Renal Clearance (converted from mL/min to L/min)
    CL_renal_L_min = params.CL_renal_5FU / 1000;
    CL_total_L_min = CL_hepatic_L_min + CL_renal_L_min;

    % 4. Calculate Half-Life
    % t1/2 = ln(2) * V / CL
    k_elim = CL_total_L_min / params.V_central; 
    estimated_halflife = log(2) / k_elim;

    fprintf('\nRESULTS:\n');
    fprintf('  Total Systemic Clearance: %.2f L/min\n', CL_total_L_min);
    fprintf('  Estimated Half-Life: %.1f minutes\n', estimated_halflife);
    fprintf('  Literature Range: 4.5 - 13 minutes (Schalhorn 1992)\n');

    if estimated_halflife >= 4.5 && estimated_halflife <= 13
        fprintf('  ✓ VALID: Matches literature data.\n');
    else
        fprintf('  ⚠️ WARNING: Outside typical range.\n');
    end
    fprintf('────────────────────────────────────────────────────────────\n\n');

    % Check 1: Is hepatic clearance less than blood flow?
    if CL_hepatic_L_min > Q_liver_L_min
        if ~isempty(logger)
            logger.error('hepatic_cl_violation', struct('CL_hep', CL_hepatic_L_min, 'Q_liver', Q_liver_L_min));
        end
        fprintf('⚠️ ERROR: CL_hep (%.3f) > Q_liver (%.2f)!\n', CL_hepatic_L_min, Q_liver_L_min);
        fprintf('This violates physiology. Aborting.\n');
        error('FATAL: Invalid clearance parameters violate Well-Stirred constraint');
    end
end
...
%  ========================================================================
%   
function dCdt = calculate5FUSystemODEs(C, t, dosingRate, DPD_factor, params, time_current, logger)
...
% Michaelis-Menten kinetics (saturable)
rate_DPD_liver_mg_min = (Vmax_current_mg_min * C_blood_entering_liver_mg_L) / ...
    (params.Km_DPD + C_blood_entering_liver_mg_L);
...
fraction_Vmax_liver = (C_blood_entering_liver_mg_L / (params.Km_DPD + C_blood_entering_liver_mg_L)) * 100;
...
rate_excrete_metabolites = ((params.CL_renal_metabolites/1000) / params.V_central) * (C_FdUMP + C_FdUTP + C_FUTP) * params.V_central;
...
dCdt.excreted_metabolites = rate_excrete_metabolites;
end
%% ========================================================================
...
function saveDetailedCSVOutputs(results, outputPrefix)
% Save comprehensive CSV files with all compartment data
    ...
    try
        ...
        logger.info('file_saved', struct('file', filename));
    catch ME
        logger.error('file_save_failed', struct('file', filename, 'message', ME.message));
        fallback = fullfile(tempdir, filename);
        try
            writetable(T, fallback);
            logger.warn('file_saved_fallback', struct('file', fallback));
        catch ME2
            logger.fatal('file_save_failed_fallback', struct('file', fallback, 'message', ME2.message));
        end
    end
    ...
    try
        ...
        logger.info('file_saved', struct('file', filename, 'purpose', 'PHYSICELL_IMPORT'));
    catch ME
        logger.error('file_save_failed', struct('file', filename, 'message', ME.message));
    end
...
function generateComprehensivePlots(results, outputPrefix)
...
if ~isfield(results, 'params') || (~isfield(results.params, 'generatePlots') || ~results.params.generatePlots)
    logger.info('plots_skipped', struct('reason', 'disabled_in_params'));
    return;
end

good_idx = 1:2:length(results.time_hr);

logger.info('generating_plots', struct('index_count', length(good_idx)));


%% Figure 1: 5-FU Distribution in Compartments (4 panels)
...
sgtitle(outputPrefix + ", 5-FU Distribution in Compartments", 'FontSize', 14, 'FontWeight', 'bold');

filename = sprintf('%s_5FU_compartments.png', outputPrefix);
saveas(gcf, filename);
savefig(gcf, sprintf('%s_5FU_compartments.fig', outputPrefix));
fprintf(' Saved: %s\n', filename);

%% Figure 2: Single Panel - All Metabolites View
figure('Position', [150, 150, 1400, 700], 'Color', 'w');
...
sgtitle(outputPrefix +"Metabolite Overview - Systemic (Top) and Tumor (Bottom)", 'FontSize', 14, 'FontWeight', 'bold');

filename = sprintf('%s_metabolites_panel.png', outputPrefix);
saveas(gcf, filename);
saveas(gcf, sprintf('%s_metabolites_panel.fig', outputPrefix));
fprintf(' Saved: %s (Single Metabolite Panel)\n', filename);
...
if ismember('dose_amount', results.dosingRegimen.Properties.VariableNames) && ~all(isnan(results.dosingRegimen.dose_amount))
    ...
    sim_dose_str = sprintf('%.0f mg bolus/infusion', sim_dose_mg);
